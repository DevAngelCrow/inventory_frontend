<template>
  <AppModal :title="props.modalState.title" :show="props.modalState.show" title-btn-cancel="Cerrar"
    title-btn-confirm="Registrar Pago" footer-buttons show-icon-close width="50rem" @close-modal="closeModal"
    @confirm-modal="onSubMit" :showBtnConfirmFooter="props.modalState.mode === 'add' && balanceDue > 0">
    ...
    <div class="flex flex-col gap-5 w-full">
      ...
      <!-- Payment Form Fields -->
      <section v-if="props.modalState.mode === 'add' && balanceDue > 0" id="body_modal"
        class="grid grid-cols-1 md:grid-cols-2 gap-5 py-1.5 w-full border-b pb-6">
        
        <AppInputMoney class="w-full min-w-0" id="amount" label="Monto a Pagar*" v-model="amount" :error-messages="errors.amount"
          v-bind="amountAttrs" :max="balanceDue" />

        <AppSelect class="w-full min-w-0" id="id_payment_method" label="Método de Pago*" v-model="id_payment_method"
          :error-messages="errors.id_payment_method" v-bind="idPaymentMethodAttrs" :options="paymentMethodsList"
          optionLabel="name" optionValue="id" />

        <AppInputText class="w-full min-w-0 md:col-span-2" id="reference_number" label="Número de Referencia / Comprobante"
          v-model="reference_number" :error-messages="errors.reference_number" v-bind="referenceNumberAttrs" />

        <AppInputextArea class="w-full min-w-0 md:col-span-2" id="notes" label="Notas de Pago" v-model="notes"
          :error-messages="errors.notes" v-bind="notesAttrs" />
      </section>

      <div v-else-if="balanceDue <= 0" class="text-center p-3 text-green-700 font-semibold bg-green-50 rounded-lg">
        Esta reserva ya se encuentra totalmente cancelada (Saldo $0.00).
      </div>

      <!-- Existing Payments List -->
      <div class="flex flex-col gap-3">
        <h4 class="font-semibold text-base border-b pb-1">Historial de Transacciones</h4>
        ...
        <div v-if="!reservationPayments.length" class="text-center py-4 text-gray-500 text-sm">
...
watch(() => props.modalState.show, async (newVal) => {
...
const onSubMit = handleSubmit(async (values) => {
  ...
  try {
    ...
    const success = await submitPayment(form);
    if (success) {
