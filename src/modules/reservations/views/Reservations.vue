<template>
  <div class="py-5 px-5 h-full max-h-full flex items-start justify-center">
    ...
    >
      <div class="w-full flex flex-row gap-3 flex-wrap items-center">
        ...
        >
          ...
          <Button
            ...
            @click="debouncedGetReservations"
            >Buscar</Button
          >
          <Button
            class="shrink-0 grow md:grow-0 rounded-md"
      ...
      >
        ...
        <template #body-balance_due="{ data }">
          <span
            :class="{ 'text-red-500 font-bold': Number(data.balance_due) > 0 }"
          >
        ...
        <template #body-mnt_customer.full_address="{ data }">
          <span
            class="block truncate max-w-[200px] hover:cursor-pointer rounded-lg bg-primary-100"
          ...
          <div class="flex gap-0 justify-start flex-wrap">
            ...
            <Button
              ...
              :disabled="
                ...
                data.status?.code !== 'CONFIRMED'
              "
            ></Button>
            <Button
              v-if="data.status?.code === 'PENDING'"
            ...
            <Button
              ...
              variant="text"
              icon="pi pi-truck"
              @click="confirmAction(data.id, 'in-progress')"
              v-tooltip.bottom="'Marcar En Progreso'"
            ></Button>
            <Button
              v-if="data.status?.code === 'IN_PROGRESS'"
            ...
            <Button
              ...
              @click="confirmAction(data.id, 'complete')"
              v-tooltip.bottom="'Completar Reserva'"
            ></Button>
            <Button
              v-if="
                data.status?.code === 'PENDING' ||
    ...
    <AppModal
      :show="cancelDialog.show"
      title="Cancelar Reserva"
      title-btn-confirm="Confirmar"
      title-btn-cancel="Cancelar"
      width="25rem"
      @close-modal="cancelDialog.show = false"
      @update:show="(val: boolean) => (cancelDialog.show = val)"
      @confirm-modal="confirmCancellation"
    >
    ...
    >
      <div class="flex flex-col gap-4 py-4">
        ...
        <div
          v-if="
            actionModal.action === 'in-progress' ||
...
const customerOptions = computed(() => {
  return [{ id: 'Todos', first_name: 'Todos' }, ...customersList.value];
});
...
const confirmAction = (
  id: string,
  action: 'confirm' | 'in-progress' | 'complete',
) => {
  const map: Record<string, { header: string; message: string }> = {
    ...
    'in-progress': {
      header: 'Marcar En Progreso',
      message: '¿Estás seguro de marcar esta reserva en progreso?',
    },
    complete: {
      header: 'Completar Reserva',
      message: '¿Estás seguro de marcar esta reserva como completada?',
    },
  };
...
const executeAction = async () => {
  ...
  if (
    (actionModal.action === 'in-progress' ||
      actionModal.action === 'complete') &&
    actionModal.datetime
  ) {
    const dateObj = CreateDateFromFormat(
      actionModal.datetime,
      'DD/MM/YYYY hh:mm a',
    );
    if (dateObj) {
