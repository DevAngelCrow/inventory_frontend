<template>
  <AppModal
    ...
    title-btn-confirm="Registrar Inspección"
    footer-buttons
    show-icon-close
    width="55rem"
    @close-modal="closeModal"
    @confirm-modal="onSubMit"
    :showBtnConfirmFooter="props.modalState.mode === 'add'"
  >
    <div class="flex flex-col gap-5 w-full">
      ...
      <section
        v-if="props.modalState.mode === 'add'"
        id="body_modal"
      ...
      >
        <AppDatePicker
          ...
          :error-messages="errors.inspection_date"
          v-bind="inspectionDateAttrs"
        />
        ...
        <AppSelect
          ...
          v-model="overall_condition"
          :error-messages="errors.overall_condition"
          v-bind="overallConditionAttrs"
        ...
        <AppInputextArea
          ...
          id="general_notes"
          label="Notas Generales de Inspección"
          v-model="general_notes"
          :error-messages="errors.general_notes"
          v-bind="generalNotesAttrs"
        />
      </section>
      ...
      <div
        v-if="props.modalState.mode === 'add'"
        class="border p-4 rounded-lg bg-gray-50 flex flex-col gap-4"
      >
        <h4 class="font-semibold text-sm border-b pb-1">
          Agregar Daño / Pérdida de Mobiliario
        </h4>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <AppSelect
            class="w-full"
            id="damage_product"
            label="Artículo de la Reserva"
            v-model="damageForm.id_product"
            :options="reservationProducts"
            optionLabel="mnt_product.name"
          ...
          <AppSelect
            ...
            id="damage_type"
            label="Tipo de Daño"
            v-model="damageForm.damage_type"
          ...
          <AppInputNumber
            ...
            id="damage_qty"
            label="Cant. Afectada"
            v-model="damageForm.quantity_affected"
          />
          ...
          <AppInputMoney
            ...
            v-model="damageForm.charge_amount"
          />

          <AppInputText
            class="w-full md:col-span-2"
            id="damage_desc"
            label="Descripción detallada del daño"
      ...
      <div class="flex flex-col gap-3">
        <h4 class="font-semibold text-base border-b pb-1">
          Artículos Dañados / Cargos Aplicados
        </h4>
        ...
        <div
          v-if="!damageItems.length"
          class="text-center py-4 text-gray-500 text-sm"
        >
          No se han registrado daños para este alquiler.
        </div>
        <div
          class="text-right font-bold text-lg text-primary mr-4"
          v-if="damageItems.length"
        >
...
const getProductName = (id_product: string) => {
  const item = reservationProducts.value.find(
    (p: ReservationItem) => p.id_product === id_product,
  );
...
const addDamage = () => {
  if (
    ...
    damageForm.quantity_affected <= 0 ||
