...
const postCustomer = async (data: CustomerForm) => {
  const response = await httpClient.post<ApiPostResponse>('customers', data);
  return response;
};
...
const toggleCustomer = async (id: string) => {
  const response = await httpClient.patch<ApiPostResponse>(`customers/${id}`);
  return response;
};

const getCustomerHistory = async (id: string) => {
  const response = await httpClient.get<
    ApiResponseGeneric<CustomerHistoryResponse>
