<template>
  <div class="py-5 px-5 h-full max-h-full flex items-start justify-center">
    <section id="geographic_divisions_content" class="w-full xl:w-[80%] flex flex-row flex-wrap gap-5">
      <AppTitle title="Divisiones Geográficas" class="w-full md:w-auto flex justify-center items-center" />
      <div id="inputs" class="flex rounded-lg py-0.5 px-0.5 gap-3 flex-wrap grow lg:grow-0 w-full">
        <AppInputText label="Buscar..." class="min-w-auto w-full sm:w-[50%] grow lg:grow-0 shrink-0 md:w-45 lg:w-83.75"
          ...
          @keydown.enter="debouncedFindDivision" />
        <AppSelect class="w-full sm:w-[20%] lg:w-auto min-w-0 grow lg:grow-0 shrink-0" :options="countries"
          option-label="name" label="País" v-model="filter.id_country" optionValue="id" />
        <AppSelect class="w-full sm:w-[20%] lg:w-auto min-w-0 grow lg:grow-0 shrink-0" :options="divisionTypesList"
          option-label="name" label="Tipo" v-model="filter.id_type" optionValue="id" />
        ...
        <Button class="shrink-0 grow rounded-md md:grow-0" @click="debouncedFindDivision">Buscar</Button>
        <Button class="shrink-0 grow md:grow-0 rounded-md" outlined @click="debouncedCleanSearch" label="Limpiar"
          :icon="iconFilter"></Button>
        <Button class="shrink-0 grow rounded-md md:grow-0 ml-auto" @click="openModal('add')"><i
      ...
      <AppDataTable class="w-full" :headers="headers" :items="divisions" :paginator="true"
    ...
    <GeographicDivisionFormModal :modal-state="modalState" @close-modal="closeModal" />
  </div>
</template>
