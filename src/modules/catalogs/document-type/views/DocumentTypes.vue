<template>
  <div class="py-5 px-5 h-full max-h-full flex items-start justify-center">
    <section id="document_type_content" class="w-full xl:w-[80%] flex flex-row flex-wrap gap-5">
      <AppTitle title="Tipos de Documento" class="w-full md:w-auto flex justify-center items-center" />
      <div id="inputs" class="flex rounded-lg py-0.5 px-0.5 gap-3 flex-wrap grow lg:grow-0 w-full">
        <AppInputText label="Buscar..." class="min-w-auto w-full sm:w-[50%] grow lg:grow-0 shrink-0 md:w-45 lg:w-83.75"
          ...
          @update:modelValue="validateAlphaInput(filter.filter_name)" @keydown.enter="debouncedFindDocumentType" />
        ...
        <Button class="shrink-0 grow md:grow-0 rounded-md" @click="debouncedFindDocumentType">Buscar</Button>
      ...
      <AppDataTable class="w-full" :headers="headers" :items="documentTypes" :paginator="true"
        ...
        <template #body-active="{ data }">
          <AppChipStatus :label="data?.status?.name" :background-color="data?.status?.state_color"
            :text-color="data?.status?.text_color"></AppChipStatus>
        </template>
      </AppDataTable>
    </section>
    <DocumentTypeFormModal :modal-state="modalState" @close-modal="closeModal" />
  </div>
</template>
