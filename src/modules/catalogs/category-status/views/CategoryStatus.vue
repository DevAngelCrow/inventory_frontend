<template>
  <div class="py-5 px-5 h-full max-h-full flex items-start justify-center">
    <section id="category_status_content" class="w-full xl:w-[80%] flex flex-row flex-wrap gap-5">
      ...
      <div id="inputs" class="flex rounded-lg py-0.5 px-0.5 gap-3 flex-wrap grow lg:grow-0 w-full">
        <AppInputText label="Buscar..." class="min-w-auto w-full sm:w-[50%] grow lg:grow-0 shrink-0 md:w-45 lg:w-83.75"
          ...
          @update:modelValue="validateAlphaInput(filter.filter_name)" @keydown.enter="debouncedFindCategoryStatus" />
        <AppSelect class="w-full sm:w-[40%] md:w-auto min-w-0 grow lg:grow-0 shrink-0" :options="statusOptions"
          option-label="name" label="Estado" v-model="filter.status" optionValue="value" />
        <Button class="shrink-0 grow md:grow-0 rounded-md" @click="debouncedFindCategoryStatus">Buscar</Button>
