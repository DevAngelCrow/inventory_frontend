<template>
  <div class="py-5 px-5 h-full max-h-full flex items-start justify-center">
    <section id="departments_content" class="w-full xl:w-[80%] flex flex-row flex-wrap gap-5">
      <AppTitle title="Categoría de Permisos" class="w-full md:w-auto flex justify-center items-center" />
      <div id="inputs" class="flex rounded-lg py-0.5 px-0.5 gap-3 flex-wrap grow lg:grow-0 w-full">
        ...
        <Button class="shrink-0 grow md:grow-0 rounded-md" @click="debouncedFindCategoryPermission">Buscar</Button>
    ...
    <CategoryPermissionFormModal :modal-state="modalState" @close-modal="closeModal" />
  </div>
</template>
