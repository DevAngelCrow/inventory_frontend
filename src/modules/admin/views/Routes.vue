<template>
  <div class="py-5 px-5 h-full max-h-full flex items-start justify-center">
    <section id="content" class="w-full xl:w-[80%] flex flex-row flex-wrap gap-5">
      <AppTitle title="Rutas" class="w-full md:w-auto flex justify-center items-center" />
      <div id="inputs" class="flex rounded-lg py-0.5 px-0.5 gap-3 flex-wrap grow lg:grow-0 w-full">
        <AppInputText label="Buscar..." class="min-w-auto w-full sm:w-[50%] grow lg:grow-0 shrink-0 md:w-45 lg:w-83.75"
          ...
          @update:modelValue="validateAlphaInput(filter.filter_name)" @keydown.enter="debouncedFindRoute" />
        <AppSelect class="min-w-0 grow lg:grow-0 shrink-0 w-full sm:w-[40%] md:w-auto" :options="statusOptions"
          option-label="name" label="Estado" v-model="filter.active" optionValue="value" />
        <AppSelect class="min-w-0 grow lg:grow-0 shrink-0 w-full sm:w-[40%] md:w-auto" :options="parentRoutes"
          option-label="name" label="Ruta padre" v-model="filter.id_parent" optionValue="id" />
        <Button class="shrink-0 grow md:grow-0 rounded-md" @click="debouncedFindRoute">Buscar</Button>
        <Button class="shrink-0 grow md:grow-0 rounded-md" outlined @click="debouncedCleanSearch" label="Limpiar"
          :icon="iconFilter"></Button>
        <Button class="shrink-0 grow md:grow-0 rounded-md ml-auto" @click="goToRouteMaintenance()"><i
      ...
      <AppDataTable class="w-full" :headers="headers" :items="items" :paginator="true" :per_page="pagination.per_page"
        ...
        :show-per-page-options="true" :per-page-options="[10, 20, 50, 100]" @per-page-update="handlePerPagePagination">
        <template #body-acciones="{ data }">
          <div class="flex gap-0 justify-center">
            <Button class="rounded-full mx-0 my-0 px-0 py-0" variant="text" icon="pi pi-eye"
              @click="goToRouteMaintenance(data.id)" v-tooltip.bottom="'Ver detalle'"></Button>
            <Button class="rounded-full" variant="text" :severity="data.active ? 'danger' : 'success'"
              icon="pi pi-trash" @click="openModal('delete', data)" v-tooltip.bottom="'Eliminar'"></Button>
          </div>
        </template>
