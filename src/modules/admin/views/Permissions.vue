    ...
    <section id="departments_content" class="w-full xl:w-[80%] flex flex-row flex-wrap gap-5">
      ...
      <div id="inputs" class="flex rounded-lg py-0.5 px-0.5 gap-3 flex-wrap grow lg:grow-0 w-full">
        <AppInputText label="Buscar..." class="min-w-auto w-full sm:w-[50%] grow lg:grow-0 shrink-0 md:w-45 lg:w-83.75"
          ...
          @update:modelValue="validateAlphaInput(filter.filter_name)" @keydown.enter="debouncedFindPermission" />
        <AppSelect class="w-full sm:w-[20%] lg:w-auto min-w-0 grow lg:grow-0 shrink-0" :options="categoryPermissions"
          option-label="name" label="Categoría" v-model="filter.category_permission_id" optionValue="id" />
        ...
        <Button class="shrink-0 grow md:grow-0 rounded-md" @click="debouncedFindPermission">Buscar</Button>
