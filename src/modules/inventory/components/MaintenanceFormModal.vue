  ...
  <AppModal :title="props.modalState.title" :show="props.modalState.show" :title-btn-cancel="modalButtons.cancelText"
    ...
    <section v-if="props.modalState.mode !== 'delete'" id="body_modal"
      ...
      <AppSelect class="w-full min-w-0" id="id_product" label="Producto*" v-model="id_product"
        :error-messages="errors.id_product" v-bind="idProductAttrs" :options="productsList"
        optionLabel="name" optionValue="id" :readonly="props.modalState.mode === 'edit' || props.modalState.isReadonly" />

      <AppInputNumber class="w-full min-w-0" id="quantity" label="Cantidad de Unidades*" v-model="quantity"
        :error-messages="errors.quantity" v-bind="quantityAttrs" :readonly="props.modalState.isReadonly" />

      <AppInputMoney class="w-full min-w-0" id="cost" label="Costo de Reparación" v-model="cost"
        :error-messages="errors.cost" v-bind="costAttrs" :readonly="props.modalState.isReadonly" />

      <AppDatePicker class="w-full min-w-0" id="date_start" label="Fecha de Inicio*" v-model="date_start"
        :error-messages="errors.date_start" v-bind="dateStartAttrs" :readonly="props.modalState.isReadonly" />

      <AppDatePicker class="w-full min-w-0" id="date_end" label="Fecha de Resolución" v-model="date_end"
        :error-messages="errors.date_end" v-bind="dateEndAttrs" :readonly="props.modalState.isReadonly" />

      <div class="w-full flex justify-start items-center py-2" v-if="props.modalState.mode === 'edit' || props.modalState.isReadonly">
        <AppCheckBox id="resolved" label="Mantenimiento Resuelto / Completado" v-model="resolved"
          :error-messages="errors.resolved" v-bind="resolvedAttrs" :disabled="props.modalState.isReadonly" />
      </div>

      <AppInputextArea class="w-full min-w-0" id="description" label="Descripción de Falla / Mantenimiento*"
        v-model="description" :error-messages="errors.description" v-bind="descriptionAttrs"
...
const onSubMit = handleSubmit(async values => {
  try {
    ...
    const endIso = values.date_end ? (FormatDateToISO(values.date_end, 'DD/MM/YYYY') || undefined) : undefined;
