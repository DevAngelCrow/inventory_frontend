<template>
  <div class="py-5 px-5 h-full max-h-full flex items-start justify-center">
    <section id="maintenance_content" class="w-full xl:w-[90%] flex flex-col flex-wrap gap-5">
      ...
      <AppDataTable class="w-full" :headers="headers" :items="maintenances" :paginator="true"
        ...
        <template #body-acciones="{ data }">
          <div class="flex gap-0 justify-center">
            ...
            <Button class="rounded-full mx-0 my-0 px-0 py-0" variant="text" icon="pi pi-pencil"
              @click="openModal('edit', data)" v-tooltip.bottom="'Editar'" :disabled="data?.resolved"></Button>
          </div>
        </template>
      </AppDataTable>
    </section>

    <MaintenanceFormModal :modal-state="modalState" @close-modal="closeModal" />
  </div>
</template>
