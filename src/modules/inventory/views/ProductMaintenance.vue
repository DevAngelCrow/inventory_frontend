      ...
      <div class="w-full flex flex-row gap-3 flex-wrap items-center">
        ...
        >
          ...
          <Button
            ...
            @click="debouncedFindMaintenance"
            >Buscar</Button
          >
          <Button
            class="shrink-0 grow md:grow-0 rounded-md"
...
const openModal = (
  ...
  switch (action) {
    ...
    case 'view':
      ...
      if (data?.date_start) {
        maintenanceInstance.setFieldValue(
          'date_start',
          FormatDate(data.date_start, 'DD/MM/YYYY'),
        );
      }
      if (data?.date_end) {
        maintenanceInstance.setFieldValue(
          'date_end',
          FormatDate(data.date_end, 'DD/MM/YYYY'),
        );
      }
      break;
    case 'edit':
...
const resolvedOptions = ref<
  { name: string; value: boolean | null | 'Todos' }[]
>([
...
const productsOptions = computed(() => {
  return [{ id: 'Todos', name: 'Todos' }, ...productsList.value];
});
