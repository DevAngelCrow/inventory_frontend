...
export const billingService = {
  ...
  getInvoiceById: async (id: string): Promise<ApiResponseGeneric<Invoice>> => {
    const response = await httpClient.get<ApiResponseGeneric<Invoice>>(`billing/invoices/${id}`);
    return response.data;
  },

  generateInvoice: async (payload: GenerateInvoicePayload): Promise<ApiResponseGeneric<Invoice>> => {
    const response = await httpClient.post<ApiResponseGeneric<Invoice>>('billing/invoices/generate', payload);
    return response.data;
  },
